...
export class ReportController {
    static async getSystemSummary(req,res){try{const d=await ReportService.getSystemSummary();res.json({success:true,data:d});}catch(e){res.status(500).json({success:false,message:e.message});}}
    static async getManagerSummary(req,res){try{const d=await ReportService.getManagerSummary();res.json({success:true,data:d});}catch(e){res.status(500).json({success:false,message:e.message});}}
