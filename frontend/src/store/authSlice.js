...
export const loginThunk = createAsyncThunk("auth/login", async ({ email, password }, { rejectWithValue }) => {
  try {
    ...
    return data;
  } catch (e) { return rejectWithValue(e.response?.data?.message || "Login dështoi"); }
});
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    logout: (state) => { state.user = null; state.token = null; state.initialized = true; },
  },
  extraReducers: (b) => {
    b.addCase(loginThunk.pending, (s) => { s.loading = true; s.error = null; })
     .addCase(loginThunk.fulfilled, (s, {payload}) => {
       s.loading = false; s.token = payload.data?.accessToken; s.user = payload.data?.user; s.initialized = true;
     })
     .addCase(loginThunk.rejected, (s, {payload}) => { s.loading = false; s.error = payload; })

     .addCase(meThunk.fulfilled, (s, {payload}) => { s.user = payload; })

     .addCase(refreshThunk.fulfilled, (s, {payload}) => { s.token = payload.data?.accessToken; s.user = payload.data?.user || s.user; s.initialized = true; })
     .addCase(refreshThunk.rejected, (s) => { s.initialized = true; });
  },
});
