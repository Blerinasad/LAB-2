...
export default function Recipes() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setItems(data.data?.items || []); setTotal(data.data?.total || 0);
  ...
  const updIng = (i, field, val) => setIngList(ingList.map((row, idx) => idx===i ? {...row,[field]:val} : row));
            ...
            <div className="mt-4">
              ...
              {ingList.map((row, i) => (
                <div key={i} className="mb-2 grid grid-cols-1 gap-2 sm:grid-cols-[minmax(0,2fr)_1fr_1fr]">
                  ...
                  <Input placeholder="njësia" value={row.unit} onChange={(e) => updIng(i,"unit",e.target.value)} style={{ flex:1 }} />
                </div>
              ))}
