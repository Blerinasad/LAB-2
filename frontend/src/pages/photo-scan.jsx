  ...
  const scan = async () => {
    ...
    try {
      ...
      if (!ingredients.length) {
        ...
        setIngredients(data.data?.items || data.data || []);
      }
    } catch (e) {
            ...
            <div className="rounded-xl bg-stone-50 dark:bg-white/[0.03] border border-stone-200 dark:border-white/[0.08] p-4 space-y-3">
              ...
              {result.demo_mode && (
                <p className="text-[11px] text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 rounded-lg px-3 py-2 font-medium">
                  Demo AI Mode: identifikimi bazohet në analizën e ngjyrave të fotos (jo AI vision reale, sepse OPENAI_API_KEY nuk është konfiguruar). Korrigjo emrin më sipër nëse nuk përputhet.
                </p>
              )}
