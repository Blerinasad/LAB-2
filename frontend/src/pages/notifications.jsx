...
import {
  setItems,
  markRead as markSlice,
  setUnread,
} from "../store/notification.slice.js";
...
const NFlame = () => (
  <svg {...nsv}>
    <path d="M10 2s.6 2.4-1.2 4.6C7.2 8.5 6 9.8 6 12a4 4 0 0 0 8 0c0-1.4-.6-2.4-1.2-3.3-.4.9-1 1.4-1 1.4.3-2.6-.6-5.7-1.8-8.1Z" />
  </svg>
);
const NSpark = () => (
  <svg {...nsv}>
    <path d="M10 2l1.6 4.4L16 8l-4.4 1.6L10 14l-1.6-4.4L4 8l4.4-1.6L10 2Zm6 8 .9 2.1L19 13l-2.1.9L16 16l-.9-2.1L13 13l2.1-.9L16 10Z" />
  </svg>
);
const NBox = () => (
  <svg {...nsv}>
    <path d="M2 5.5 10 2l8 3.5v9L10 18l-8-3.5v-9Zm8 1.6L4.6 4.9 10 3l5.4 1.9L10 7.1Zm-6 1 5 2.2v6.3l-5-2.2V8.1Zm12 0v6.3l-5 2.2v-6.3l5-2.2Z" />
  </svg>
);
const TYPE = {
  expiry_alert: {
  ...
  system: {
    ...
    bg: "bg-stone-50 dark:bg-white/[0.04] border-stone-200 dark:border-white/[0.06]",
  },
};
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-5">
        <div className="flex items-center gap-2.5">
          <span className="text-[13px] text-stone-500 dark:text-stone-500">
            {items.length} njoftime
          </span>
          {unread > 0 && (
            <span className="badge badge-orange">{unread} të reja</span>
          )}
        </div>
        {unread > 0 && (
          <button className="btn-ghost btn-sm" onClick={markAll}>
      ...
      {items.length === 0 ? (
        <div className="sk-card">
          <Empty
            ...
            sub="Njoftimet e reja do shfaqen këtu"
          />
        </div>
      ) : (
        <div className="flex flex-col gap-2.5">
          {items.map((n) => {
            ...
            return (
              ...
              >
                <div className={`text-xl flex-shrink-0 mt-0.5 ${t.color}`}>
                  {t.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-[13px] font-semibold text-stone-800 dark:text-stone-200 ${!n.is_read ? "font-bold" : ""}`}
                  >
                    {n.title}
                  </p>
