...
export default function MealPlans() {
  ...
  useEffect(() => {
    api
      ...
      .then(({ data }) => setRecipes(data.data?.items || []));
  }, []);

  const submit = async (e) => {
    e.preventDefault();
    try {
      await api.post("/meal-plans", form);
      toast.success("Plani u krijua");
      setModal(false);
      load();
    } catch (e) {
      toast.danger("Gabim", e.response?.data?.message);
    }
  };
  ...
  const generateShopping = async (planId) => {
    ...
    try {
      const { data } = await api.post(
        `/meal-plans/${planId}/generate-shopping`,
      );
      if (data.data.shopping_list_id) {
        toast.success(
          "Lista u gjenerua",
          `${data.data.missing_count} artikuj mungojnë. Hape te Lista Blerje.`,
        );
        sessionStorage.setItem(
          "sk_open_shopping_list",
          String(data.data.shopping_list_id),
        );
        window.dispatchEvent(
          new CustomEvent("sk:navigate", { detail: "shopping" }),
        );
      } else {
        toast.info(
          "Nuk ka nevojë për blerje",
          "Inventari i mbulon të gjithë ingredientët e këtij plani.",
        );
      }
    } catch (e) {
      toast.danger("Gabim", e.response?.data?.message);
              ...
              <tbody>
                {plans.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td>
                      <Badge variant={STATUS_COLOR[p.status]}>
        ...
        <Modal
          ...
          actions={
            <>
              ...
              <button className="btn-primary" form="mp-form" type="submit">
                Krijo
              </button>
            </>
          }
      ...
      {detail && (
            ...
            <div className="flex flex-wrap gap-2">
              {detail.status !== "active" && (
                <button
                  ...
                  onClick={() => updateStatus(detail, "active")}
                >
                  Aktivizo
                </button>
              )}
              ...
              <button
                className="btn-secondary btn-sm"
                onClick={() => generateShopping(detail.id)}
                disabled={generating}
              >
                {generating ? "..." : "Gjenero Blerjet"}
              </button>
        ...
        >
          <p className="mb-4 text-[11px] text-stone-400">
            {fmtDate(detail.week_start)} - {fmtDate(detail.week_end)} ·{" "}
            <Badge variant={STATUS_COLOR[detail.status]}>
          ...
          {detail.days?.length === 0 ? (
            <Empty
              icon="—"
              title="Nuk ka ditë"
              sub='Kliko "+ Shto Ditë" për të shtuar vaktet'
            />
          ) : (
            <div className="flex flex-col gap-2">
              {detail.days.map((d) => (
                <div
                  key={d.id}
                  className="flex flex-wrap items-center gap-3 rounded-xl bg-stone-50 px-3 py-2.5 dark:bg-white/[0.04]"
                >
                  <span className="min-w-[80px] text-[10px] font-bold uppercase tracking-wide text-orange-500">
                    {DAYS_ALB[(d.day_of_week || 1) - 1]}
                  </span>
                  <span className="min-w-[60px] text-[10px] text-stone-400">
                    {MEAL_LABELS[d.meal_type]}
                  </span>
                  <span className="flex-1 text-[12px] font-medium text-stone-700 dark:text-stone-300">
                    {d.recipe_title}
                  </span>
        ...
        <Modal
          ...
          actions={
            <>
              ...
              <button className="btn-primary" form="day-form" type="submit">
                Shto
              </button>
            </>
          }
        >
          <form id="day-form" onSubmit={addDay}>
            <FormGroup label="Receta">
              <Select
                value={dayForm.recipe_id}
                onChange={(e) =>
                  setDayForm({ ...dayForm, recipe_id: e.target.value })
                }
                required
              >
            ...
            <FormRow>
              <FormGroup label="Dita e Javës">
                <Select
                  ...
                  onChange={(e) =>
                    setDayForm({ ...dayForm, day_of_week: e.target.value })
                  }
                >
                  {DAYS_ALB.map((d, i) => (
              ...
              <FormGroup label="Vakti">
                <Select
                  value={dayForm.meal_type}
                  onChange={(e) =>
                    setDayForm({ ...dayForm, meal_type: e.target.value })
                  }
                >
            ...
            <FormGroup label="Servime">
              <Input
                type="number"
                min="1"
                value={dayForm.servings}
                onChange={(e) =>
                  setDayForm({ ...dayForm, servings: e.target.value })
                }
              />
            </FormGroup>
          </form>
